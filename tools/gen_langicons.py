#!/usr/bin/env python3
"""Download Devicon SVGs for termio's language and tool icon catalog.

Run from the repo root. Writes loose SVGs into Sources/termio/LangIcons/ for macOS,
an asset catalog into ios/Sources/LangIcons.xcassets (UIImage cannot decode SVG at
runtime, so iOS ships them Xcode-compiled), and the shared catalog into
Shared/Sources/TermioShared/LangIconCatalog.swift.
"""
import json, os, sys, urllib.request, concurrent.futures, re

REPO = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
ICON_DIR = os.path.join(REPO, "Sources/termio/LangIcons")
...
def main():
    ...
    def fetch(item):
        ...
        out = os.path.join(ICON_DIR, f"{name}.svg")
    ...
    lines.append("// Maps a file's extension or exact name to a bundled Devicon mark —")
    lines.append("// a loose SVG on macOS (Sources/termio/LangIcons/), an asset-catalog")
    lines.append("// imageset on iOS (ios/Sources/LangIcons.xcassets). Devicon is MIT;")
    ...
    for name in sorted(have):
        ...
        os.makedirs(d)
        shutil.copy(os.path.join(ICON_DIR, f"{name}.svg"), os.path.join(d, f"{name}.svg"))
        contents = {
