...
| `resume` | | Resume strategy: `"none"` (default), `"claude"`, `"codex"`, `"pi"`, or `"opencode"`. |
...
A line matching `attention` flips the session to the **attention** state (it wins
over `working`, since a prompt can sit under a still-spinning header); a line
matching `working` marks it **working**. See the [sidebar status
reference](/docs/sidebar#status) for what each state means.
