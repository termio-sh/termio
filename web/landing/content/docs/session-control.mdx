...
`termio sessions` is the short path over one plane of
[the session protocol](/docs/session-protocol) — the verbs a person or a hook
would type, with the session you meant worked out from where you are. When you
need a verb it does not wrap, or a connection your own program holds open, that
page picks up where this one stops.
...
agents. Open a machine under **Settings ▸ Devices** and press **Set Up** — or
**Reinstall** on the Hooks row — to put the hooks on that box. That is what gives
...
Turn on **Settings ▸ Devices ▸ This Mac ▸ Command-line tool**. It symlinks a `termio`
